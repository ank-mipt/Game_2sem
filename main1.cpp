...
protected:
    ...
    float currentFrame;
public:
    PRADED(Texture& image) : dx(0), dy(0), currentFrame(0) {
...
class PLAYER : public PRADED {
private:
    bool onGround;
public:
    PLAYER(Texture& image) : PRADED(image), onGround(true) {
...
class ENEMY : public PRADED {
private:
    bool isAlive;

public:
    ENEMY() : PRADED(), isAlive(false) {}
